...
const getRedisConnection = () => {
  ...
  try {
    ...
    console.warn(
      "[queue]: Invalid REDIS_URI, falling back to default localhost",
    );
...
export const initializeQueues = () => {
  if (!isRedisConnected()) {
    console.warn(
      "[queue]: Redis not connected. BullMQ queues will not be initialized.",
    );
    return;
  }
  ...
  try {
    ...
    notificationQueueEvents.on("failed", ({ jobId, failedReason }) => {
      console.error(
        `[queue]: Notification job ${jobId} failed — ${failedReason}`,
      );
    });
