...
const getDatabaseUri = () => {
  ...
  if (!uri) {
    console.error(
      "[database]: FATAL ERROR: MONGO_URI environment variable is not defined.",
    );
    process.exit(1);
  }
  return uri;
...
export const connectDB = async () => {
  ...
  try {
    ...
    console.log(
      `[database]: Mongoose connection established with host: ${conn.connection.host}`,
    );
  } catch (error) {
    console.error(
      `[database]: Failed to connect to primary MongoDB: ${error.message}`,
    );
    // Fallback to MongoMemoryServer for development if Atlas connection fails
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        "[database]: Falling back to MongoMemoryServer for local development...",
      );
      try {
        ...
        console.log(
          `[database]: Mongoose connected to Memory Server: ${conn.connection.host}`,
        );
      } catch (fallbackError) {
        console.error(
          `[database]: Critical failure - Fallback to Memory Server also failed: ${fallbackError.message}`,
        );
        process.exit(1);
      }
    } else {
...
export const handleGracefulShutdown = () => {
  const gracefulExit = async (signal) => {
    ...
    try {
      ...
      console.error(
        "[database]: Error closing MongoDB connection during shutdown:",
        err,
      );
      process.exit(1);
    }
