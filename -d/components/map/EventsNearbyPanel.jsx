...
export const EventsNearbyPanel = () => {
  ...
  const { toggles, selectedLocationId, searchQuery } = useAppSelector(
    (state) => state.map,
  );
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {filteredEvents.map((event) => (
          ...
          >
            <div className="flex justify-between items-start mb-2">
              <h3
                className={`font-bold text-sm ${selectedLocationId === event._id ? "text-indigo-300" : "text-gray-200"}`}
              >
                {event.title}
              </h3>
            ...
            <AnimatePresence>
              {selectedLocationId === event._id && (
                ...
                >
                  <Link
                    to={`/events/${event._id}`}
                    className="flex-1 text-center py-2 bg-indigo-600 hover:bg-indigo-500 text-gray-900 dark:text-white font-bold text-xs rounded-lg transition-colors"
                  >
                    View Details
                  </Link>
        ...
        {filteredEvents.length === 0 && (
          <div className="h-40 flex flex-col items-center justify-center text-gray-500 text-center px-4 border border-dashed border-gray-200 dark:border-gray-800 rounded-2xl">
            ...
            <p className="text-sm">
              No mapped events match your filters right now.
            </p>
          </div>
        )}
      </div>
