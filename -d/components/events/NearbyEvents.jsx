...
const NearbyEvents = () => {
  const { coordinates, loading, error, requestLocation, permission } =
    useGeolocation();
  ...
  if (loading || isLoading)
    return (
      <div className="animate-pulse h-40 bg-black/5 dark:bg-white/5 rounded-xl"></div>
    );
    ...
    return (
      <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex flex-col items-center justify-center text-center">
        ...
        <button
          ...
          className="text-xs bg-red-500/20 hover:bg-red-500/30 text-red-300 py-1.5 px-3 rounded-lg transition-colors"
        >
          Retry / Grant Access
        </button>
      </div>
