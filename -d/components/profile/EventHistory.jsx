...
export const EventHistory = ({ events }) => {
  ...
  const upcomingEvents = events.filter(
    (e) => new Date(e.event.startDate) >= now,
  );
    ...
    <div className="border border-gray-200 dark:border-gray-800 rounded-xl bg-white dark:bg-gray-900/40 overflow-hidden shadow-sm">
      <div className="flex border-b border-gray-200 dark:border-gray-800">
        <button
          ...
          className={cn(
            "flex-1 py-3 text-sm font-medium transition-colors border-b-2 cursor-pointer",
            tab === "upcoming"
              ? "border-primary-500 text-gray-900 dark:text-white bg-gray-50 dark:bg-gray-800/50"
              : "border-transparent text-gray-600 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 hover:bg-gray-800/30",
          )}
        >
            ...
            {displayList.map((e) => (
              ...
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="text-gray-900 dark:text-white font-semibold group-hover:text-primary-400 transition-colors">
                      {e.event.title}
                    </h4>
                    <div className="flex items-center gap-3 text-xs text-gray-600 dark:text-gray-400 mt-2">
                      <span className="flex items-center">
                        <Calendar className="w-3.5 h-3.5 mr-1" />{" "}
                        {new Date(e.event.startDate).toLocaleDateString()}
                      </span>
