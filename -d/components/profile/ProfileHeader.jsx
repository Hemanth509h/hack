    ...
    <div className="relative pt-40 pb-12 glass rounded-t-[3rem] overflow-hidden border-black/5 dark:border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.5)]">
      ...
      <div className="max-w-6xl mx-auto px-10 relative z-10 flex flex-col md:flex-row items-center md:items-end gap-10">
        ...
        <div className="flex-1 pb-2 text-center md:text-left">
          ...
          <h1 className="text-4xl md:text-6xl font-display font-black text-gray-900 dark:text-white tracking-tighter leading-none">
            {profile.name}
          </h1>
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 mt-6 text-gray-500 text-xs font-black uppercase tracking-widest">
            {profile.major && (
              <span className="flex items-center gap-2 bg-black/5 dark:bg-white/5 px-4 py-2 rounded-xl border border-black/5 dark:border-white/5">
            ...
            {profile.graduationYear && (
              <span className="flex items-center gap-2 bg-black/5 dark:bg-white/5 px-4 py-2 rounded-xl border border-black/5 dark:border-white/5">
                <Briefcase className="w-4 h-4 text-purple-500" /> Class of{" "}
                {profile.graduationYear}
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 pb-2 mt-8 md:mt-0">
          {isOwner ? (
            <>
              <Link
                to="/profile/edit"
                className="btn-glass px-6 py-4 border-black/10 dark:border-white/10 text-gray-900 dark:text-white font-black uppercase tracking-widest text-[10px]"
              >
                <Edit3 className="w-4 h-4 mr-2 inline-block" /> Edit Identity
              </Link>
              <Link
                to="/portfolio/edit"
                className="btn-primary px-8 py-4 shadow-indigo-500/30 text-[10px] font-black uppercase tracking-widest"
              >
                Portfolio Builder
              </Link>
            </>
