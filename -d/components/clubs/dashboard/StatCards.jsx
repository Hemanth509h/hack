      ...
      {stats.map((stat, idx) => (
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div
              className={`p-3 rounded-2xl bg-${stat.color}-500/10 text-${stat.color}-400 group-hover:scale-110 transition-transform`}
            >
              {stat.icon}
            </div>
            {stat.change && (
              <span
                className={`text-[10px] font-bold uppercase tracking-wider ${stat.change.includes("+") ? "text-green-400" : "text-gray-500"}`}
              >
                {stat.change}
              </span>
            )}
