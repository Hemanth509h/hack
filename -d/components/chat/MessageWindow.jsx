...
export const MessageWindow = ({
  ...
  const displaySubtitle =
    subtitle ||
    (roomType === "direct" ? otherUser?.major || "Student" : "Group Chat");
  const displayAvatar =
    avatar ||
    otherUser?.avatar ||
    `https://ui-avatars.com/api/?name=${encodeURIComponent(displayTitle)}`;
    ...
    <div className="flex flex-col h-full bg-gray-100 dark:bg-gray-950">
        ...
        {messages.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center">
            ...
            <p className="text-gray-500 text-sm max-w-xs">
              Start the conversation by sending a friendly "Hello!"
            </p>
          </div>
        ) : (
          messages.map((msg, i) => {
      ...
      <div className="p-4 bg-white dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-800">
        <form onSubmit={handleSubmit} className="flex items-center gap-3">
          <button
            type="button"
            className="p-2 text-gray-500 hover:text-gray-900 dark:text-white transition-colors"
          >
            <Paperclip size={20} />
          </button>
          ...
          <div className="flex-1 relative">
            ...
            <button
              type="button"
              className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-900 dark:text-white transition-colors"
            >
              <Smile size={20} />
            </button>
          </div>
