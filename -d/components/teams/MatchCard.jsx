...
const matchColors = {
  ...
  mid: {
    badge: "from-violet-500 to-indigo-400",
...
export const MatchCard = ({ match, rank, onViewDetails }) => {
  ...
  return (
    ...
    >
      {/* Match % Badge */}
      <div
        className={`absolute -top-3 -right-3 bg-gradient-to-br ${colors.badge} text-gray-900 dark:text-white text-xs font-black px-3 py-1.5 rounded-xl shadow-lg z-10`}
      >
        {pct}% Match
      </div>
      ...
      <div className="flex items-start gap-4 mb-4">
        {/* Avatar */}
        <div
          className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${avatarGradient} flex items-center justify-center text-gray-900 dark:text-white font-bold text-lg shrink-0 shadow-lg`}
        >
          {match.user.avatar ? (
            <img
      ...
      <div className="space-y-2 mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-600 dark:text-gray-400 w-8 text-right">
            {match.breakdown.interestAlignment}%
          </span>
        </div>
      </div>
