...
export const getAllEvents = async (req, res) => {
  try {
    const { page, limit, search, category, startDate, endDate, club, sortBy } =
      req.query;
  ...
  try {
    if (req.user?.role !== "admin") {
      return res
        .status(403)
        .json({
          error: "Only administrators can initialize official campus events.",
        });
    }
    ...
    if (!organizerId) {
      return res
        .status(400)
        .json({ error: "An event organizer must be assigned." });
    }
    ...
    res
      .status(201)
      .json({ message: "Event created and matches notified", event: newEvent });
  } catch (error) {
    res
      .status(500)
...
export const rsvpToEvent = async (req, res) => {
  try {
    ...
    if (event.status !== "published") {
      return res
        .status(400)
        .json({ error: "Cannot RSVP to an unpublished event" });
    }
    ...
    res
      .status(201)
      .json({
        message: "RSVP request sent. Awaiting organizer approval.",
        rsvp: newRsvp,
      });
  } catch (error) {
  ...
  try {
    ...
    if (!signature) {
      ...
      if (!targetUserId)
        return res
          .status(400)
          .json({ error: "QR Payload or Manual userId required" });
    } else {
      ...
      if (signature !== expectedSig) {
        return res
          .status(403)
          .json({ error: "Invalid or forged QR signature." });
      }
      ...
      if (qrAge > 24 * 60 * 60 * 1000) {
        return res
          .status(400)
          .json({ error: "QR Code expired. Please refresh your code." });
      }
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Not authorized to operate check-ins" });
    }
    ...
    if (!rsvp)
      return res
        .status(404)
        .json({ error: "User does not have an RSVP for this event" });
    if (rsvp.isCheckedIn)
...
export const getEventAttendees = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Not authorized to view attendee list" });
    }
...
export const approveRSVP = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only the event organizer can approve RSVPs." });
    }
    ...
    const updatedEvent = await Event.findById(req.params.id).select(
      "rsvpCount",
    );
...
export const rejectRSVP = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only the event organizer can reject RSVPs." });
    }
...
export const getNearbyEvents = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch nearby events", details: error.message });
  }
};
  ...
  try {
    ...
    const icsEvent = {
      ...
      location: event.location?.name
        ? `${event.location.name} ${event.location.buildingCode || ""}`
      ...
      organizer: {
        ...
        email: event.organizer?.email || "noreply@quad.edu",
      },
...
export const getQrCodeData = async (req, res) => {
  try {
    ...
    if (!rsvp)
      return res
        .status(403)
        .json({ error: "You do not have an active RSVP for this event." });
