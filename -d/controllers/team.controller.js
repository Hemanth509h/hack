...
export const getMatches = async (req, res) => {
  try {
    ...
    const myCompletedMemberLists = myCompletedProjects.map((p) => [
      ...p.members.map(String),
      String(p.leader),
    ]);
    ...
    const openProjectLeaders = await TeamProject.find({
      status: "open",
    }).distinct("leader");
    ...
    res
      .status(500)
      .json({ error: "Matching engine failed", details: error.message });
  }
};
...
export const updateProject = async (req, res) => {
  try {
    ...
    if (
      String(project.leader) !== req.user.userId &&
      req.user.role !== "admin"
    ) {
      return res
        .status(403)
        .json({ error: "Only the project leader can update this project" });
    }

    const updated = await TeamProject.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true },
    );
...
export const deleteProject = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only the project leader can delete this project" });
    }
...
export const requestToJoin = async (req, res) => {
  try {
    ...
    if (project.status !== "open") {
      return res
        .status(400)
        .json({ error: "Project is not open for new members" });
    }
...
export const acceptRequest = async (req, res) => {
  try {
    ...
    if (String(project.leader) !== req.user.userId) {
      return res
        .status(403)
        .json({ error: "Only the project leader can accept requests" });
    }
...
export const declineRequest = async (req, res) => {
  try {
    ...
    if (String(project.leader) !== req.user.userId) {
      return res
        .status(403)
        .json({ error: "Only the project leader can decline requests" });
    }
...
export const browseProjects = async (req, res) => {
  try {
    ...
    if (event) query.associatedEvent = new mongoose.Types.ObjectId(event);
