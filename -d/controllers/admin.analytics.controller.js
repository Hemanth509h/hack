...
export const getEngagementTrends = async (req, res) => {
  try {
    ...
    const trends = await adminService.getEngagementAnalytics(
      parseInt(days) || 30,
    );
    res.json(trends);
  } catch (error) {
...
export const downloadEngagementReport = async (req, res) => {
  try {
    const { days } = req.body;
    const { rsvpAnalytics } = await adminService.getEngagementAnalytics(
      days || 30,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=engagement_report_${new Date().toISOString()}.csv`,
    );
