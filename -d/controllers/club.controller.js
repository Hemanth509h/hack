...
export const createClub = async (req, res) => {
  try {
    if (req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only administrators can initialize new clubs." });
    }
    ...
    if (!headId) {
      return res
        .status(400)
        .json({ error: "A student head must be assigned to the club." });
    }
    ...
    if (!headUser) {
      return res
        .status(404)
        .json({ error: "Assigned student head not found." });
    }
    ...
    res
      .status(201)
      .json({ message: "Club created and head assigned successfully", club });
  } catch (error) {
    res
      .status(500)
...
export const updateClub = async (req, res) => {
  try {
    ...
    if (!membership && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Not authorized to configure club" });
    }
  ...
  try {
    ...
    if (existing) {
      return res
        .status(400)
        .json({
          error: `You already have a ${existing.status} request/membership`,
        });
    }
    ...
    res
      .status(201)
      .json({
        message: "Join request sent. Awaiting leadership approval.",
        membership,
      });
  } catch (error) {
...
export const updateMemberRole = async (req, res) => {
  try {
    ...
    if (!commander && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only club heads can modify member structures" });
    }

    const { role } = req.body;

    // Find and update
    const targetMember = await ClubMembership.findOne({
...
export const approveMember = async (req, res) => {
  try {
    ...
    if (!isHead && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only the club head can approve members." });
    }
...
export const rejectMember = async (req, res) => {
  try {
    ...
    if (!isHead && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Only the club head can reject members." });
    }
...
export const getClubAnalytics = async (req, res) => {
  try {
    ...
    if (!isLeader && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Analytics strictly locked to internal leadership" });
    }
    ...
    const eventsPipeline = await Event.aggregate([
      { $match: { club: clubId } },
      {
        $group: {
          _id: "$status",
          totalEvents: { $sum: 1 },
          totalRSVPsGained: { $sum: "$rsvpCount" },
        },
      },
    ]);
    ...
    res
      .status(500)
      .json({ error: "Analytics process failed", details: error.message });
  }
};
