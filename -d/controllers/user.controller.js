...
export const addSkill = async (req, res) => {
  try {
    if (req.user?.userId !== req.params.id) {
      return res
        .status(403)
        .json({ error: "Cannot modify another user's skills" });
    }
...
export const updateInterests = async (req, res) => {
  try {
    if (req.user?.userId !== req.params.id) {
      return res
        .status(403)
        .json({ error: "Cannot modify another user's interests" });
    }

    // Deduplicate interests before saving
    const deduped = [
      ...new Set(req.body.interests.map((i) => i.toLowerCase().trim())),
    ];
...
export const uploadAvatar = async (req, res) => {
  try {
    if (req.user?.userId !== req.params.id && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Cannot update another user's avatar" });
    }
    ...
    res
      .status(500)
      .json({ error: "Avatar upload failed", details: error.message });
  }
};
  ...
  try {
    const [user, rsvps, memberships] = await Promise.all([
      User.findById(req.params.id)
        .select(
          "name avatar bio major graduationYear skills interests portfolioLinks",
        )
        .populate("skills", "name category"),
      RSVP.find({ user: req.params.id, isCheckedIn: true })
        .populate("event", "title date category")
        .limit(20),
      ClubMembership.find({ user: req.params.id, status: "approved" }).populate(
        "club",
        "name logo category",
      ),
    ]);
...
export const updateLocation = async (req, res) => {
  try {
    if (req.user?.userId !== req.params.id && req.user?.role !== "admin") {
      return res
        .status(403)
        .json({ error: "Cannot update another user's location" });
    }
