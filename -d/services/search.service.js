...
export const trackSearch = async (userId, query, type) => {
  try {
    // Avoid spamming history with identical queries
    const lastSearch = await SearchHistory.findOne({ user: userId }).sort({
      timestamp: -1,
    });
...
export const getAutocompleteSuggestions = async (q) => {
  ...
  const [events, clubs] = await Promise.all([
    Event.find({ status: "published", title: regex })
      .limit(5)
      .select("title category")
      .lean(),
    Club.find({ status: "active", name: regex })
      .limit(5)
