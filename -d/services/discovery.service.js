...
export const getPersonalizedRecommendations = async (userId) => {
  ...
  const collaborativeResults = await RSVP.find({
    ...
    .populate("event")
    .then((rsvps) =>
      rsvps.map((r) => r.event).filter((e) => !!e && e.status === "published"),
    );

  // Merge and deduplicate
  const allResults = [...contentResults, ...collaborativeResults];
  const uniqueResults = Array.from(
    new Map(allResults.map((item) => [String(item._id), item])).values(),
  );
