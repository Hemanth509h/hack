...
export const processChatMessage = async (userId, userMessage, res) => {
  try {
    ...
    if (EVENT_KEYWORDS.some((k) => msg.includes(k))) {
      responseText =
        "I've searched the campus calendar for you. Here are some upcoming events:";
      ...
      responseText =
        "Here is the campus information I found regarding your query:";
      dataResults = await aiFunctions.getCampusInfo({ topic: userMessage });
    } else if (GREETINGS.some((k) => msg.includes(k))) {
      responseText =
        "Hello! I'm Nexus, your campus assistant. I can help you find events, clubs, or campus information. What's on your mind?";
    } else {
      responseText =
        "I'm not quite sure about that, but I can help you find clubs, events, or general campus info! Try asking 'What events are happening today?'";
    }
    ...
    res
      .status(500)
      .write(
        "I'm having trouble accessing the campus database right now. Please try again later.",
      );
    res.end();
  }
};
