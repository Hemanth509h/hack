...
export const renderEmailHtml = (templateName, context) => {
  try {
    ...
    if (!compiledTemplate) {
      ...
      if (!fs.existsSync(filePath)) {
        console.warn(
          `[email-renderer]: Template ${templateName}.hbs not found. Falling back to explicit strings.`,
        );
        return "";
      }
    ...
    console.error(
      `[email-renderer]: Failure rendering ${templateName}:`,
      error,
    );
