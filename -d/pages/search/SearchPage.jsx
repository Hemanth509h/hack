    ...
    const fetchData = async () => {
      ...
      try {
        const { data } = await api.get(
          `/search?q=${encodeURIComponent(rawQ)}&type=${type}`,
        );
      ...
      <div className="max-w-full md:px-12 lg:px-20 mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-3">
          <form
            ...
            className="relative mb-8 shadow-xl"
          >
          ...
          {rawQ && (
            <div className="mb-6 pb-6 border-b border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 font-medium">
              {isLoading
                ? "Scanning matrix..."
                : `Displaying ${total} optimal matches for "${rawQ}"`}
            </div>
          )}
          ...
          {isLoading ? (
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {/* Image or Logo block */}
                      <div
                        className={`w-16 h-16 rounded-xl flex items-center justify-center overflow-hidden flex-shrink-0 ${isEvent ? "bg-indigo-900/40 text-indigo-400" : "bg-pink-900/40 text-pink-400"}`}
                      >
                        {item.coverImage || item.logo ? (
                          <img
                            src={item.coverImage || item.logo}
                            alt={isEvent ? item.title : item.name}
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-1">
                          <h3 className="text-xl font-bold text-gray-200 group-hover:text-gray-900 dark:text-white transition">
                            {isEvent ? item.title : item.name}
                          </h3>
                          <span
                            className={`text-[10px] uppercase font-black tracking-wider px-2 py-0.5 rounded border ${isEvent ? "border-indigo-500/30 text-indigo-400" : "border-pink-500/30 text-pink-400"}`}
                          >
                            {isEvent ? "Event" : "Club"}
                          </span>
                        </div>
            ...
            <div className="py-24 text-center">
              ...
              <p className="text-gray-500">
                Check your spelling or drop the filters.
              </p>
            </div>
          ) : null}
        </div>
