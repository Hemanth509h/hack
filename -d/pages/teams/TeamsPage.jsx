      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 text-lg">
            Collaborate on hackathons, research, and campus initiatives.
          </p>
        </div>
            ...
            {browseLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className="h-64 bg-black/5 dark:bg-white/5 animate-pulse rounded-3xl border border-black/5 dark:border-white/5"
                  />
                ))}
              </div>
            ...
            <div className="bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 rounded-3xl p-8 mb-8 relative overflow-hidden">
              <div className="relative z-10">
                ...
                <p className="text-blue-100/70 max-w-2xl">
                  Our algorithm analyzes your skills, interests, and past
                  collaborations to suggest the perfect teammates for your next
                  big project.
                </p>
              </div>
            ...
            {matchesLoading ? (
              <div className="space-y-4">
                {[1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="h-24 bg-black/5 dark:bg-white/5 animate-pulse rounded-2xl border border-black/5 dark:border-white/5"
                  />
                ))}
              </div>
                ...
                {myProjectsData.projects.map((project) => (
                  ...
                  >
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h3 className="font-bold text-gray-900 dark:text-white group-hover:text-blue-400 transition-colors">
                          {project.title}
                        </h3>
    ...
    <div className="space-y-4 pt-6 border-t border-black/5 dark:border-white/5">
      <div className="flex flex-wrap gap-2">
        {project.requiredSkills.slice(0, 3).map((s, i) => (
          <span
            key={i}
            className="text-[10px] bg-black/5 dark:bg-white/5 text-gray-600 dark:text-gray-400 px-2 py-1 rounded-lg border border-black/5 dark:border-white/5"
          >
            {s.skill.name}
          </span>
        ))}
  ...
  <div className="p-6 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-3xl hover:bg-black/10 dark:hover:bg-white/10 transition-all">
    <div className="flex items-center space-x-4 mb-6">
      <img
        src={
          match.user.avatar ||
          `https://api.dicebear.com/7.x/avataaars/svg?seed=${match.user.name}`
        }
      ...
      <div className="p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/5">
        ...
        <div className="h-1.5 w-full bg-gray-50 dark:bg-gray-800 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${match.breakdown.skillOverlap}%` }}
          />
        </div>
      </div>
      <div className="p-3 bg-black/5 dark:bg-white/5 rounded-xl border border-black/5 dark:border-white/5">
        ...
        <div className="h-1.5 w-full bg-gray-50 dark:bg-gray-800 rounded-full overflow-hidden">
          <div
            ...
            style={{ width: `${match.breakdown.interestAlignment}%` }}
          />
        </div>
      </div>
    </div>
    ...
    <div className="flex items-center justify-between">
      <div className="flex flex-wrap gap-1">
        {match.user.skills.slice(0, 2).map((s) => (
          <span
            key={s._id}
            className="text-[10px] bg-blue-500/10 text-blue-400 px-2 py-0.5 rounded-lg"
          >
            {s.name}
          </span>
        ))}
