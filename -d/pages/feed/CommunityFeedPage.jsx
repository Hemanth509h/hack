      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-12">
          ...
          <p className="text-gray-600 dark:text-gray-400 text-lg">
            Real-time updates from clubs, events, and projects across campus.
          </p>
        </div>
        ...
        {isLoading ? (
          <div className="space-y-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-48 bg-white dark:bg-gray-900/50 border border-gray-200 dark:border-gray-800 rounded-3xl animate-pulse"
              />
            ))}
          </div>
        ) : (
...
const FeedCard = ({ item, index }) => {
  const Icon =
    item.type === "event" ? Calendar : item.type === "club" ? Users : Briefcase;
  const colorClass =
    item.type === "event"
