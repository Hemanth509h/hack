      ...
      <div className="relative mb-24 rounded-[3rem] overflow-hidden glass p-10 md:p-20 flex flex-col md:flex-row items-center justify-between gap-12 border-black/5 dark:border-white/5 shadow-[0_40px_100px_rgba(0,0,0,0.4)]">
        ...
        <div className="relative z-10 max-w-2xl text-center md:text-left">
          ...
          <p className="text-gray-600 dark:text-gray-400 text-xl mb-10 leading-relaxed font-light max-w-lg">
            Find the perfect group that shares your passion and elevates your
            campus experience through shared excellence.
          </p>
          <div className="flex flex-wrap items-center gap-5 justify-center md:justify-start">
            {user?.role === "admin" && (
              <Link
                to="/clubs/create"
                className="btn-primary shadow-indigo-500/40 px-10"
              >
            ...
            <Link
              to="/clubs/my-clubs"
              className="btn-glass px-10 border-black/10 dark:border-white/10"
            >
              My Clubs
            </Link>
          </div>
        ...
        <div className="relative z-10 hidden lg:block">
          ...
          >
            ...
            <div className="text-gray-900 dark:text-white font-display font-black text-2xl mb-2 tracking-tighter">
              Impact Award
            </div>
            <p className="text-gray-500 text-sm font-medium leading-relaxed">
              Honoring the club that drives the most campus engagement this
              semester.
            </p>
          </motion.div>
        </div>
      ...
      <div className="mb-12 flex flex-col md:flex-row items-end justify-between gap-6 px-4">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-3">
            Showing {clubsData?.pagination.total || 0} elite organizations
          </p>
        </div>
        <ClubFilters />
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="h-96 glass rounded-[2.5rem] animate-pulse opacity-50"
            />
          ))}
        </div>
      ) : (
        <>
          ...
          {clubsData?.clubs.length === 0 && (
            <div className="text-center py-32 glass rounded-[4rem] border-dashed border-black/10 dark:border-white/10">
              ...
              <h3 className="text-3xl font-display font-black text-gray-500 tracking-tighter">
                No Clubs Found
              </h3>
          ...
          {clubsData && clubsData.pagination.pages > 1 && (
            <div className="mt-24 flex justify-center gap-3">
              {Array.from({ length: clubsData.pagination.pages }).map(
                (_, i) => (
                  <button
                    key={i}
