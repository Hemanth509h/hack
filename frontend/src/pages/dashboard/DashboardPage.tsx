...
import { Calendar, Users, Briefcase, Sparkles, Map, ChevronLeft, ChevronRight } from 'lucide-react';
import { 
...
const DashboardPage: React.FC = () => {
  ...
  const activeCarouselIndex = useAppSelector((state) => state.dashboard.activeCarouselIndex);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        ...
        <div className="xl:col-span-8 space-y-10">
          ...
          <section className="flex flex-wrap gap-3">
             <Link to="/map" className="flex items-center gap-2 bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 px-4 py-2.5 rounded-xl text-sm font-bold transition-colors border border-blue-500/20">
                <Map className="w-4 h-4" /> Interactive Campus Map
             </Link>
             <Link to="/teams" className="flex items-center gap-2 bg-violet-500/10 hover:bg-violet-500/20 text-violet-400 px-4 py-2.5 rounded-xl text-sm font-bold transition-colors border border-violet-500/20">
            ...
            {trendingLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative overflow-hidden">
                <AnimatePresence mode="popLayout">
                  {trendingEvents.slice(0, 2).map((event: any, idx: number) => (
                    <motion.div
                      key={event._id}
        ...
        <div className="xl:col-span-4 space-y-10">
          ...
          <section className="bg-gray-900/40 rounded-3xl p-6 border border-white/5">
            ...
            <div className="space-y-6">
               ...
               <div>
                  <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4">Clubs To Explore</h3>
