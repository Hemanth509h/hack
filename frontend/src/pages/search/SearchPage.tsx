    ...
    <div className="min-h-screen bg-gray-950 pt-24 pb-20">
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-8">
        ...
        <div className="md:col-span-1 space-y-6">
           <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 shadow-xl relative top-8 sticky">
              ...
              <div className="space-y-4">
                 ...
                 <div className="flex flex-col gap-2">
                   <button 
                     ...
                     className={`text-left px-3 py-2 rounded-lg text-sm transition font-medium ${type === 'all' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'text-gray-400 hover:bg-gray-800'}`}
                   >
                     Everything
                   </button>
                   <button 
                     onClick={() => handleTypeChange('event')}
                     className={`flex items-center gap-2 text-left px-3 py-2 rounded-lg text-sm transition font-medium ${type === 'event' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'text-gray-400 hover:bg-gray-800'}`}
                   >
                     <CalendarIcon size={16} /> Events Only
                   </button>
                   <button 
                     onClick={() => handleTypeChange('club')}
                     className={`flex items-center gap-2 text-left px-3 py-2 rounded-lg text-sm transition font-medium ${type === 'club' ? 'bg-blue-600/20 text-blue-400 border border-blue-500/30' : 'text-gray-400 hover:bg-gray-800'}`}
                   >
        ...
        <div className="md:col-span-3">
           <form onSubmit={handleSearchCommit} className="relative mb-8 shadow-xl">
             ...
             <input
               ...
               className="w-full bg-gray-900 border border-gray-800 rounded-2xl py-4 pl-14 pr-6 text-xl text-white placeholder-gray-500 outline-none focus:border-blue-500 transition shadow-inner"
               placeholder="Search the entire campus..."
             />
             <button 
               type="submit" 
               className="absolute right-3 top-1/2 -translate-y-1/2 bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-xl font-bold transition"
             >
               Search
             </button>
           </form>
           ...
           {/* Metrics */}
           {rawQ && (
              <div className="mb-6 pb-6 border-b border-gray-800 text-gray-400 font-medium">
           ...
           {isLoading ? (
                 ...
                 {results.map((item, i) => {
                    ...
                    return (
                      ...
                      >
                       <Link 
                         to={destination}
                         className="flex flex-col md:flex-row gap-4 p-5 bg-gray-900/50 hover:bg-gray-800 border border-gray-800 hover:border-gray-700 rounded-2xl transition group"
                       >
                          {/* Image or Logo block */}
                          ...
                          <div className="flex-1">
                             <div className="flex items-center gap-3 mb-1">
                                <h3 className="text-xl font-bold text-gray-200 group-hover:text-white transition">{isEvent ? item.title : item.name}</h3>
             ...
             <div className="py-24 text-center">
                 ...
                 <h2 className="text-2xl font-bold text-white mb-2">No results matched</h2>
