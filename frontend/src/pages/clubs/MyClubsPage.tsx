        ...
        <div className="flex bg-gray-950 p-1.5 rounded-2xl border border-gray-800">
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
              activeTab === 'leading' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${
              activeTab === 'member' ? 'bg-indigo-600 text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'
            }`}
          >
          ...
          >
            {data?.leading && data.leading.length > 0 ? (
              data.leading.map((club) => (
                <div key={club._id} className="relative group">
                  ...
                  <Link 
                    ...
                    className="absolute bottom-4 right-20 bg-white/10 backdrop-blur-md border border-white/20 text-white px-4 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-white/20 transition-all z-20"
                  >
              ...
              <div className="col-span-full py-20 text-center bg-gray-900/30 rounded-3xl border border-dashed border-gray-800">
                ...
                <h3 className="text-xl font-bold text-gray-400">Not leading any clubs</h3>
                <p className="text-gray-500 mt-2 mb-8">Ready to take charge? Start your own movement today.</p>
                <Link to="/clubs/create" className="inline-flex items-center gap-2 bg-indigo-600 text-white px-8 py-4 rounded-2xl font-bold hover:bg-indigo-500 transition-all shadow-xl">
          ...
          >
            {data?.memberOf && data.memberOf.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center bg-gray-900/30 rounded-3xl border border-dashed border-gray-800">
                ...
                <h3 className="text-xl font-bold text-gray-400">No active memberships</h3>
