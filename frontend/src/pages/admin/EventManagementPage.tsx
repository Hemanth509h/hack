    ...
    <div className="space-y-10 py-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
         <div>
            ...
            <h1 className="text-4xl md:text-5xl font-display font-black mb-3 tracking-tighter text-gray-900 dark:text-white">Event Governance</h1>
            <p className="text-gray-500 font-medium text-lg">Oversee, edit, and coordinate all campus-wide occurrences.</p>
         </div>
         <Link 
           to="/events/create" 
         ...
         <div className="overflow-x-auto">
            <table className="w-full text-left">
               ...
               <tbody className="divide-y divide-black/5 dark:divide-white/5">
                  {isLoading ? (
                    <tr>
                      <td colSpan={5} className="py-20 text-center text-gray-500 font-bold uppercase tracking-widest text-xs">Synchronizing Event Streams...</td>
                    </tr>
                  ) : events.map((event: any) => (
                    <tr key={event._id} className="group hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                       <td className="py-6 pl-4">
                          <div className="flex items-center gap-4">
                             <div className="w-12 h-12 rounded-xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 font-bold overflow-hidden border border-black/10 dark:border-white/10">
                                {event.coverImage ? <img src={event.coverImage} className="w-full h-full object-cover" /> : event.title.charAt(0)}
                             </div>
                             <div>
                                <div className="text-gray-900 dark:text-white font-bold tracking-tight">{event.title}</div>
                                <div className="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-0.5">{event.category}</div>
                             </div>
                          </div>
                       </td>
                       <td className="py-6">
