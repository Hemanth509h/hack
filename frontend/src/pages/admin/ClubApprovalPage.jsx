...
import { Calendar, Users, X, Check, MessageSquare, ShieldCheck, PlusCircle } from 'lucide-react';
    ...
    <div className="space-y-10 py-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
         <div>
            ...
            <h1 className="text-4xl md:text-5xl font-display font-black mb-3 tracking-tighter text-gray-900 dark:text-white">Clubs Management</h1>
            <p className="text-gray-500 font-medium text-lg">Initialize new organizations or moderate the pending queue.</p>
         </div>
         <div className="flex items-center gap-4">
            ...
            <div className="bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-2xl px-6 py-3 flex items-center gap-4">
               <div className="text-right">
                  ...
                  <div className="text-gray-900 dark:text-white font-black text-xl">{pendingClubs.length} Pending</div>
               </div>
      ...
      {isLoading ? (
        ...
        >
           ...
           <h3 className="text-3xl font-display font-black text-gray-900 dark:text-white mb-4 tracking-tighter">Excellence Maintained</h3>
        ...
        <div className="grid gap-10 md:grid-cols-2">
          {pendingClubs.map((app, index) => (
            ...
            >
               ...
               <h3 className="text-3xl font-display font-black text-gray-900 dark:text-white mb-4 pr-24 tracking-tighter leading-tight">{app.name}</h3>
               ...
               <div className="grid grid-cols-2 gap-6 mb-10">
                  <div className="glass bg-black/5 dark:bg-white/5 rounded-2xl p-5 border-black/5 dark:border-white/5">
                    ...
                    <div className="text-gray-900 dark:text-white text-lg font-black">{new Date(app.applicationDate).toLocaleDateString()}</div>
                  </div>
                  <div className="glass bg-black/5 dark:bg-white/5 rounded-2xl p-5 border-black/5 dark:border-white/5">
                    ...
                    <div className="text-gray-900 dark:text-white text-lg font-black">{app.expectedMembership} Elite</div>
                  </div>
               </div>
               ...
               <div className="mb-10">
                 ...
                 <div className="space-y-3">
                   {app.proposedLeaders.map((leader, i) => (
                     <div key={i} className="flex justify-between items-center glass bg-white/2 px-5 py-4 rounded-2xl border-black/5 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                       <span className="text-base font-bold text-gray-900 dark:text-white tracking-tight">{leader.name}</span>
                 ...
                 {selectedApp === app.id && (
                   ...
                   >
                      ...
                      <div className="flex gap-4">
                         <button onClick={() => setSelectedApp(null)} className="flex-1 py-4 glass bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-gray-900 dark:text-white rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all border-black/10 dark:border-white/10">
                           Cancel
                         </button>
                         <button onClick={() => handleReject(app.id)} disabled={!rejectReason.trim()} className="flex-1 py-4 bg-red-600 hover:bg-red-500 disabled:opacity-50 text-gray-900 dark:text-white rounded-2xl font-black uppercase tracking-widest text-[11px] transition-all shadow-lg shadow-red-500/20">
                           Confirm Rejection
                         </button>
                      </div>
                   </motion.div>
