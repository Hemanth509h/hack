...
const StatCard = ({ title, value, subValue, change, isPositive, icon: Icon, alert = false }: any) => (
  <motion.div 
    ...
    className={cn(
      "glass rounded-[2.5rem] p-8 shadow-[0_20px_40px_rgba(0,0,0,0.3)] relative overflow-hidden border-white/5",
  ...
  >
    ...
    <div className="flex justify-between items-start relative z-10">
      <div>
        ...
        <h3 className="text-4xl font-display font-black text-white tracking-tighter leading-tight">{value}</h3>
      </div>
      <div className={cn(
        "p-4 rounded-2xl shadow-lg",
        alert ? 'premium-gradient text-white' : 'glass bg-white/5 text-indigo-400 border-white/10'
      )}>
        <Icon size={24} />
      </div>
    </div>
    ...
    <div className="space-y-12 py-6">
      <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-8 px-4">
        <div className="text-center md:text-left">
           ...
           <h1 className="text-5xl md:text-7xl font-display font-black text-white mb-4 tracking-tighter leading-[0.9]">
             Campus <br />
        ...
        <div className="flex gap-4">
           <button className="btn-glass px-8 py-4 border-white/10 text-white font-black uppercase tracking-widest text-xs">
              Generate Report
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 glass rounded-[3rem] p-10 shadow-[0_30px_60px_rgba(0,0,0,0.4)] relative overflow-hidden border-white/5">
           <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-6">
              <div>
                <h3 className="text-3xl font-display font-black text-white tracking-tighter">Engagement Velocity</h3>
                <p className="text-gray-500 font-medium mt-1">Real-time RSVP volume across {dateRange.toLowerCase()}ly periods.</p>
              </div>
              <div className="glass bg-white/5 p-1 rounded-2xl border-white/10">
                <select 
                  value={dateRange}
                  onChange={(e) => setDateRange(e.target.value)}
                  className="bg-transparent text-white text-[10px] font-black uppercase tracking-widest p-3 pr-8 focus:outline-none cursor-pointer"
                >
                  <option value="Week" className="bg-[#030303]">Weekly Period</option>
                  <option value="Month" className="bg-[#030303]">Monthly Period</option>
                </select>
              </div>
           </div>
        ...
        <div className="flex flex-col gap-10">
          <div className="glass rounded-[3rem] p-10 shadow-2xl border-white/5 relative overflow-hidden">
             ...
             <h3 className="text-2xl font-display font-black text-white mb-8 tracking-tighter">Strategic Actions</h3>
             <div className="space-y-4">
                <button className="w-full flex justify-between items-center glass bg-white/5 hover:bg-white/10 text-white rounded-2xl px-6 py-5 transition-all duration-300 border-white/5 hover:border-indigo-500/30">
                  <span className="font-black text-xs uppercase tracking-widest">Review Club Forms</span>
                  <div className="flex items-center gap-3">
                    <span className="bg-indigo-600 text-white text-[10px] font-black px-3 py-1 rounded-full shadow-[0_0_15px_rgba(99,102,241,0.4)]">{stats?.newClubRequests?.total || 0}</span>
                  </div>
                </button>
                <button className="w-full flex justify-between items-center glass bg-white/5 hover:bg-white/10 text-white rounded-2xl px-6 py-5 transition-all duration-300 border-white/5">
          ...
          <div className="premium-gradient rounded-[3rem] p-10 shadow-[0_30px_60px_rgba(99,102,241,0.3)] relative overflow-hidden flex flex-col items-center text-center">
             ...
             <h3 className="text-2xl font-display font-black text-white mb-4 tracking-tighter">Admin Codex</h3>
