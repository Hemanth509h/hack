...
export const clubApi = api.injectEndpoints({
  endpoints: (builder) => ({
    fetchClubs: builder.query<PaginatedClubResponse, ClubFilter>({
      query: (params) => ({
        url: '/clubs',
        params,
      }),
    ...
    fetchFeaturedClubs: builder.query<FeaturedClubsResponse, void>({
      query: () => '/clubs/featured',
      providesTags: ['Club'],
    }),
    fetchClubById: builder.query<ClubDetailResponse, string>({
      query: (id) => `/clubs/${id}`,
    ...
    updateClub: builder.mutation<IClub, { id: string; updates: Partial<IClub> }>({
      query: ({ id, updates }) => ({
        url: `/clubs/${id}`,
        method: 'PATCH',
        body: updates,
    ...
    joinClub: builder.mutation<IClubMember, string>({
      query: (clubId) => ({
    ...
    fetchClubMembers: builder.query<ClubMembersResponse, string>({
      query: (clubId) => `/clubs/${clubId}/members`,
      providesTags: (_result, _error, clubId) => [{ type: 'Club', id: `${clubId}-members` }],
    }),
    updateMemberRole: builder.mutation<IClubMember, { clubId: string; userId: string; role: string }>({
    ...
    approveMember: builder.mutation<IClubMember, { clubId: string; userId: string }>({
      query: ({ clubId, userId }) => ({
    ...
    rejectMember: builder.mutation<void, { clubId: string; userId: string }>({
      query: ({ clubId, userId }) => ({
    ...
    fetchClubAnalytics: builder.query<ClubAnalytics, string>({
      query: (clubId) => `/clubs/${clubId}/analytics`,
    }),
    fetchMyClubs: builder.query<{ leading: IClub[]; memberOf: IClub[] }, void>({
