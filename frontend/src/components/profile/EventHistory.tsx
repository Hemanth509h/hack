...
export const EventHistory: React.FC<{ events?: any[] }> = ({ events }) => {
  ...
  return (
    <div className="border border-gray-800 rounded-xl bg-gray-900/40 overflow-hidden shadow-sm">
      <div className="flex border-b border-gray-800">
        <button
          onClick={() => setTab('upcoming')}
          className={cn("flex-1 py-3 text-sm font-medium transition-colors border-b-2 cursor-pointer",
            tab === 'upcoming' ? "border-primary-500 text-white bg-gray-800/50" : "border-transparent text-gray-400 hover:text-gray-300 hover:bg-gray-800/30"
          )}
        >
        ...
        <button
          onClick={() => setTab('past')}
          className={cn("flex-1 py-3 text-sm font-medium transition-colors border-b-2 cursor-pointer",
            tab === 'past' ? "border-primary-500 text-white bg-gray-800/50" : "border-transparent text-gray-400 hover:text-gray-300 hover:bg-gray-800/30"
          )}
        >
          ...
          <div className="space-y-3">
            {displayList.map(e => (
              <Link 
                ...
                className="block p-4 bg-gray-950 border border-gray-800 rounded-lg hover:border-gray-700 transition-colors group"
              >
                <div className="flex items-start justify-between">
                  <div>
                    <h4 className="text-white font-semibold group-hover:text-primary-400 transition-colors">{e.event.title}</h4>
