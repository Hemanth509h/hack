...
export const MatchCard: React.FC<MatchCardProps> = ({ match, rank, onViewDetails }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative group bg-gray-900/70 backdrop-blur-xl border ${colors.border} rounded-2xl p-6 cursor-pointer transition-all duration-300 hover:shadow-xl hover:${colors.glow} hover:-translate-y-1`}
      onClick={() => onViewDetails(match)}
    >
      {/* Match % Badge */}
      <div className={`absolute -top-3 -right-3 bg-gradient-to-br ${colors.badge} text-white text-xs font-black px-3 py-1.5 rounded-xl shadow-lg z-10`}>
        {pct}% Match
      </div>
      ...
      <div className="flex items-start gap-4 mb-4">
        {/* Avatar */}
        <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${avatarGradient} flex items-center justify-center text-white font-bold text-lg shrink-0 shadow-lg`}>
          {match.user.avatar
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-white font-bold text-base truncate">{match.user.name}</h3>
      ...
      <div className="flex flex-wrap gap-1.5 mb-5">
        ...
        {match.user.skills.length > 5 && (
          <span className="text-[11px] font-semibold text-gray-500 bg-gray-800 px-2.5 py-1 rounded-lg">
            +{match.user.skills.length - 5}
          </span>
        )}
      </div>
      ...
      <div className="space-y-2 mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 w-8 text-right">{match.breakdown.skillOverlap}%</span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 w-8 text-right">{match.breakdown.interestAlignment}%</span>
        </div>
      </div>
      ...
      <button
        ...
        className={`w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 ${
          hovered
            ? `bg-gradient-to-r ${colors.badge} text-white shadow-lg`
