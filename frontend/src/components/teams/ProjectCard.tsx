...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, isLeader, index = 0 }) => {
    ...
    >
      ...
      <div className="flex items-start justify-between mb-3 gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <span className={`text-[11px] font-bold px-2.5 py-1 rounded-lg border ${status.color}`}>
            {status.label}
          </span>
        </div>
      </div>
      ...
      {project.requiredSkills.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.requiredSkills.slice(0, 4).map((rs, i) => (
            <span key={i} className="text-[11px] font-semibold text-violet-300 bg-violet-500/10 border border-violet-500/20 px-2.5 py-0.5 rounded-lg flex items-center gap-1">
              {rs.proficiencyDesired === 'expert' ? <CheckCircle className="w-2.5 h-2.5" /> : <Circle className="w-2.5 h-2.5" />}
              {rs.skill.name}
            </span>
          ))}
