...
const QRScanner = ({ eventId }: QRScannerProps) => {
  ...
  const handleManualCheckin = async (e: React.FormEvent) => {
    ...
    try {
       ...
       setScanResult('error');
       setResultMessage(err.data?.error || 'User not found or already checked in');
       setTimeout(() => {
         setScanResult('idle');
         setIsProcessing(false);
       }, 4000);
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden shadow-2xl relative max-w-md mx-auto">
       <div className="flex bg-gray-950 p-1 border-b border-gray-800 relative z-10">
          <button 
            onClick={() => setManualMode(false)}
            className={`flex-1 py-3 text-sm font-bold flex flex-center gap-2 transition ${!manualMode ? 'text-white bg-gray-900 rounded-lg shadow' : 'text-gray-500 hover:text-gray-300'}`}
          >
       ...
       <div className="relative aspect-square bg-black">
          {!manualMode ? (
             ...
             <div className="absolute inset-0 bg-gray-900 flex flex-col items-center justify-center p-6 text-center">
                ...
                <p className="text-gray-400 text-sm mb-6">Enter User ID or Student Number to bypass scanner.</p>
                <form onSubmit={handleManualCheckin} className="w-full flex gap-2">
                   <input 
                     type="text" 
                     value={manualUserId}
                     onChange={(e) => setManualUserId(e.target.value)}
                     className="flex-1 bg-gray-950 border border-gray-700 rounded-xl px-4 py-3 text-gray-200 outline-none focus:border-blue-500 transition"
                     placeholder="Enter exact ID..."
                   />
                   <button 
                     type="submit" 
                     disabled={!manualUserId.trim() || isProcessing}
                     className="bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white px-4 rounded-xl font-bold transition"
                   >
                     Submit
                   </button>
                </form>
            ...
            {scanResult !== 'idle' && (
              <motion.div 
                ...
                className={`absolute inset-0 z-20 flex flex-col items-center justify-center p-6 text-center backdrop-blur-md ${
                  scanResult === 'success' ? 'bg-green-500/80 text-white' : 'bg-red-500/80 text-white'
                }`}
              >
                ...
                <p className="font-semibold text-white/90 drop-shadow">{resultMessage}</p>
              </motion.div>
            )}
          </AnimatePresence>
       </div>
