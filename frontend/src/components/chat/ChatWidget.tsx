        ...
        {!isOpen && (
           ...
           >
             <button 
               ...
               className="w-14 h-14 rounded-full bg-gradient-to-r from-purple-600 to-blue-600 text-white flex items-center justify-center shadow-lg shadow-purple-500/30 hover:scale-105 transition-transform group relative"
             >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-6 right-6 z-[100] w-[380px] h-[600px] max-h-[80vh] flex flex-col bg-gray-900 rounded-2xl border border-gray-800 shadow-2xl overflow-hidden"
          >
            {/* Header */}
            <div className="h-16 bg-gradient-to-r from-purple-900/40 to-blue-900/40 border-b border-gray-800 flex flex-row justify-between items-center px-4">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                   ...
                   >
                     {msg.sender === 'bot' ? (
                       <div className="prose prose-invert prose-p:leading-relaxed prose-pre:bg-gray-900 prose-sm max-w-none">
                         ...
                         {msg.richContent && (
                            <div className="mt-3 bg-gray-900 p-3 rounded-lg border border-gray-700 text-xs flex justify-between items-center">
                              <div>
                                <p className="font-semibold text-gray-100">{msg.richContent.title || 'Event Details'}</p>
                                <p className="text-gray-400">{msg.richContent.subtitle || 'Information Card'}</p>
                              </div>
              ...
              {isTyping && (
                <div className="flex justify-start">
                  ...
                  <div className="bg-gray-800 border border-gray-700 text-gray-400 rounded-2xl rounded-tl-sm px-4 py-3 flex gap-1.5 items-center">
            ...
            <div className="p-3 bg-gray-900 border-t border-gray-800">
                 ...
                 <div className="flex gap-2 mb-3 overflow-x-auto pb-1 no-scrollbar">
                   {["Where is the Seminar Hall?", "Library hours today?", "Next basketball match?"].map((q, i) => (
                     <button
                       ...
                       className="whitespace-nowrap flex-shrink-0 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 text-xs text-gray-300 rounded-full border border-gray-700 transition"
                     >
                       {q}
                     </button>
                   ))}
               ...
               <div className="relative flex items-end gap-2 bg-gray-950 p-2 rounded-xl border border-gray-700 focus-within:border-purple-500/50 transition-colors">
                  ...
                  <textarea
                    ...
                    className="flex-1 max-h-32 min-h-10 bg-transparent text-sm text-gray-100 placeholder-gray-500 focus:outline-none resize-none py-2"
                    rows={1}
                  />
                  <button 
                    onClick={handleSend}
                    disabled={!inputText.trim()}
                    className="bg-blue-600 disabled:opacity-50 disabled:hover:scale-100 text-white p-2 rounded-lg hover:scale-105 transition-transform"
                  >
                    <Send size={16} />
                  </button>
               </div>
