...
export const NotificationItem: React.FC = ({ notification }) => {
  ...
  return (
    <div
      onClick={handleRead}
      className={cn(
        "flex p-4 border-b border-gray-200 dark:border-gray-800 hover:bg-gray-800/50 cursor-pointer transition-colors",
        !notification.isRead ? "bg-gray-50 dark:bg-gray-800/30" : "bg-transparent"
      )}
    >
      <div className="flex-shrink-0 mt-1">
        <div className="w-10 h-10 rounded-full bg-white dark:bg-gray-900 flex items-center justify-center border border-gray-700 shadow-inner">
          {getIcon()}
        </div>
      </div>
      <div className="ml-4 flex-1">
        <div className="flex items-start justify-between">
          <p className={cn("text-sm", !notification.isRead ? "text-gray-900 dark:text-white font-semibold" : "text-gray-700 dark:text-gray-300 font-medium")}>
            {notification.title}
          </p>
        ...
        <p className={cn("text-sm mt-1", !notification.isRead ? "text-gray-700 dark:text-gray-300" : "text-gray-600 dark:text-gray-400")}>
