...
const rateLimitMap = new Map<string, { count; resetAt: number }>();
...
export const roomId = (type: 'event' | 'club' | 'project' | 'user' | 'direct', id) =>
  `${type}:${id}`;
...
export const initializeSocket = async (httpServer) => {
  ...
  io.on('connection', (socket) => {
    ...
    // TYPING INDICATORS
    // ----------------------------------------------------------------
    socket.on('chat:typing', (payload: { roomType; roomId: string }) => {
...
export const emitToRoom = (type: 'event' | 'club' | 'project' | 'direct', id, event, data) => {
  if (!io) return;
 ...
 */
export const emitRsvpUpdate = (eventId, rsvpCount, action: 'added' | 'cancelled') => {
 ...
 */
export const emitNewEventMatch = (userIds: string[], event) => {
  if (!io) return;
