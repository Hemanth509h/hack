...
export const findClubs = async (args) => {
    try {
        const clubs = await Club.find({ status: 'active', $text: { $search: args.query } }, { score: { $meta: 'textScore' } })
...
export const getCampusInfo = async (args) => {
    try {
        const info = await CampusInfo.find({ $text: { $search: args.topic } }, { score: { $meta: 'textScore' } })
            .sort({ score: { $meta: 'textScore' } })
            .limit(1);
