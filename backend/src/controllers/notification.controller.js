...
export const markAsRead = async (req, res) => {
    try {
        const notification = await Notification.findOneAndUpdate({ _id: req.params.id, recipient: req.user.userId }, { isRead: true }, { new: true });
        if (!notification)
...
export const updatePreferences = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.user.userId, { $set: { notificationPreferences: { email, push, inApp } } }, { new: true }).select('notificationPreferences');
