...
import { processAndUploadImage } from '../utils/upload.utils.js';
...
export const updateProfile = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { $set: req.body }, { new: true, runValidators: true }).select('-password -resetPasswordToken -resetPasswordExpires');
        if (!user)
            return res.status(404).json({ error: 'User not found' });
...
export const addSkill = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { $addToSet: { skills: new mongoose.Types.ObjectId(skillId) } }, { new: true }).populate('skills', 'name category');
...
export const removeSkill = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { $pull: { skills: new mongoose.Types.ObjectId(req.params.skillId) } }, { new: true }).populate('skills', 'name category');
        if (!user)
            return res.status(404).json({ error: 'User not found' });
...
export const updateInterests = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { $set: { interests: deduped } }, { new: true });
...
export const uploadAvatar = async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { $set: { avatar: avatarUrl } }, { new: true }).select('name avatar');
