...
export const getMatches = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const mySkillNames = (me.skills as any[]).map((s: any) => s.name);
    ...
    const myCompletedMemberLists = myCompletedProjects.map((p) =>
      [...p.members.map(String), String(p.leader)]
    );
    ...
    const scored = await Promise.all(
      candidates.map(async (candidate) => {
        const theirSkillNames = (candidate.skills as any[]).map((s: any) => s.name);
 ...
 */
export const browseProjects = async (req: AuthRequest, res: Response) => {
  try {
    const { page = 1, limit = 10, skill, event } = req.query as any;

    const query: any = { status: 'open' };
    if (skill) query['requiredSkills.skill'] = new mongoose.Types.ObjectId(skill);
